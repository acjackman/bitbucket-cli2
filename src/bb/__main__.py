...
class BitBucketClient:
    ...
    def start_pipeline(self, branch_name, pipeline_name, extras):
        ...
        return self._post("/pipelines/", json=payload).json()

    def wait(  # noqa: C901
        self, pipeline: dict, sleep_time=15, watchdog_max=100
    ) -> Optional[bool]:
...
def user_callback():
    try:
        return keyring.get_password("bitbucket-cli2", "username")
    except Exception:
        log.warn("Unable to get Bitbucket username from keychain")
        return None
...
def password_callback():
    try:
        return keyring.get_password("bitbucket-cli2", "password")
    except Exception:
        log.warn("Unable to get Bitbucket password from keychain")
        return None
